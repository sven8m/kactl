 ...
 * powers, repeatedly set $A^{-1} = A^{-1} (2I - AA^{-1})\  (\text{mod }p^k)$
 * where $A^{-1}$ starts as the inverse of A mod p, and k is doubled in each
 * step. Time: $O(n^3)$ Status: Slightly tested
 */
#pragma once
...
int matInv(vector<vector<double>> &A) {
  ...
  rep(i, 0, n) {
    int r = i, c = i;
    rep(j, i, n) rep(k, i, n) if (fabs(A[j][k]) > fabs(A[r][c])) r = j, c = k;
    if (fabs(A[r][c]) < 1e-12)
      return i;
    A[i].swap(A[r]);
    tmp[i].swap(tmp[r]);
